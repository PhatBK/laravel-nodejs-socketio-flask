...
def similarity_cosine(person1, person2, datasets):
    both_viewed = {}
    un_both_viewed = {}
    set_both_viewed = {}
    for item in datasets[person1]:
        if item in datasets[person2]:
            both_viewed[item] = 1
        else:
            un_both_viewed[item] = 0
        if len(both_viewed) == 0:
            return 0
        sum_of_cosine_distance = []
    print(both_viewed)
    print(un_both_viewed)
...
def most_similar_users(person, number_of_users, dataset):
    ...
    scores = [(pearson_correlation(person, other_person, dataset), other_person) for other_person in dataset if
              other_person != person]
...
def user_reommendations_knn(person, dataset, similarity_min, rate_threshold):
	...
	for other in dataset:
		...
		if sim <= 0 or sim < similarity_min:
			continue
